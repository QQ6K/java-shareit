...
public class BookingController {
    ...
    public BookingDtoExport patchBooking(@Valid @NotNull(message = "Отсутсвует X-Sharer-User-Id") @RequestHeader("X-Sharer-User-Id")
                                         long userId, @PathVariable Long bookingId, @RequestParam Boolean approved) {
        log.info("Запрос 'PATCH /bookings/{}'", bookingId);
        return bookingService.updateBooking(userId, bookingId, approved);
    }

    @GetMapping("/{bookingId}")
    public Booking readById(@PathVariable long bookingId) {
    ...
    public Collection<Booking> readAllOwner(@RequestHeader("X-Sharer-User-Id") @NotNull(message = "Отсутсвует X-Sharer-User-Id")
                                            Long userId) {
        log.info("Запрос 'GET /items' пользователя " + userId);
        return bookingService.readAllOwner(userId);
    }
